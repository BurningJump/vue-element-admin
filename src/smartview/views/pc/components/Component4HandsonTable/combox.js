...
(function(Handsontable) {
  ...
  ComboxEdit.prototype.open = function() {
    ...
    const _cellProp = this.cellProperties
    const _enumModelItems = _cellProp['enumModel'].items

    const _objValue = this.instance.getDataAtCell(this.row, this.col)
    for (const item of _enumModelItems) {
  ...
  function showEditorDisplayValue(_this) {
    const editor = _this.getActiveEditor()

    let _cellValue = ''
    const _cellProp = editor.cellProperties
    const _objValue = _this.getDataAtCell(editor.row, editor.col)
    const _enumModelItems = _cellProp['enumModel'].items
